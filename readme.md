...
This is an unofficial Home Assistant integration for Argo climate control devices, using the undocumented API used by the webapp.

> **This is a fork.** The original integration was written by
> [@nyffchanium](https://github.com/nyffchanium)
> ([nyffchanium/argoclima-integration](https://github.com/nyffchanium/argoclima-integration)).
> This fork adds compatibility with newer Home Assistant releases and makes the
> local communication a bit more robust. See
> [Changes in this fork](#changes-in-this-fork) for details. All credit for the
> original work goes to the upstream author.
...
- **Home Assistant 2026.x compatibility**
  - Removed the deprecated `verify_domain_control` service wrapper; the
    `set_time` service is registered directly.
  - Device registry is accessed via `dr.async_get(hass)` instead of the
    internal data registry.
  - `async_setup` uses `ConfigType` instead of the removed
    `homeassistant.core_config.Config` import.
- **More robust local API (`api.py`)**
  - Request timeout raised to 15 s.
  - Requests to the device are serialized with an `asyncio.Lock`.
  - A `User-Agent` header is sent.
  - Debug logging of the request URL and the raw device response.
- **Update coordinator** tolerates up to 3 consecutive failed polls (keeping the
  last known data) before marking the device unavailable — Argo units
  occasionally drop single requests.
- **Manifest**: `integration_type: device`, version `1.2.0`.
- **HACS**: installs directly from source (no release ZIP asset required).
...
3. Add `https://github.com/xHecktor/argoclima-integration` with category **Integration** and confirm.
4. Search for **Argoclima** in HACS, open it and click **Download**.
5. Restart Home Assistant.
6. Go to **Settings** → **Devices & Services**, click **Add Integration** and search for **Argoclima**.

Once GitHub releases are published on this repository, you can pick a specific
version in HACS via the download / redownload dialog.
...
7. In the HA UI go to "Settings" -> "Devices & Services" click "+" and search for "Argoclima"
